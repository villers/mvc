...
class JsonFileCache implements AutoloadCacheManager
{
    ...
    {
        if( !$this->loaded && file_exists( PATH_TMP . self::FILENAME ) )
            $this->cache = json_decode( file_get_contents( PATH_TMP . self::FILENAME ), true );
    ...
    public function save()
    {                                
        file_put_contents( PATH_TMP . self::FILENAME, json_encode( $this->cache ) );
    }
}
